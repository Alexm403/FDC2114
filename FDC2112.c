...
uint16_t FDC2112_Get_Value_Chanel(defInitConfigStructFDC *init_struct, uint8_t chanel)
{
	uint16_t sensor1 = 0x00;

	uint8_t TXbuffer[1] = {0,};
	uint8_t RXbuffer[4] = {0,};

	TXbuffer[0] = chanel * 2; //ADDRES_DATA0_REG;
	HAL_I2C_Master_Transmit(init_struct->i2c_fdc, init_struct->addr, (uint8_t*)TXbuffer, 1, 1000);
	HAL_I2C_Master_Receive (init_struct->i2c_fdc, init_struct->addr, (uint8_t*)&RXbuffer[0], 4, 1000);

	sensor1 = ( RXbuffer[1]<<0 |RXbuffer[0]<<8 ) & 0x0FFF;

	return sensor1;
}

uint16_t FDC2112_Get_Reg (defInitConfigStructFDC *init_struct, uint8_t reg)
{
	uint16_t sensor1 = 0x00;
